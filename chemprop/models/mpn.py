...
from Attention import attention, MultiHeadAttention, TransformerEncoder, Parametric_attention
...
class MPNEncoder(nn.Module):
    ...
    def forward(self,
        ...
        # concatenate the atom descriptors - (computed features of the molecule)
        if atom_descriptors_batch is not None:
        ...
        for i, (a_start, a_size) in enumerate(a_scope):
            if a_size == 0:
                ...
                elif self.aggregation == 'self_attention': #w/o learnable weights in the self-attention mechanism
                    mol_vec_old, attn_weights = attention(mol_vec, mol_vec, mol_vec)
                    mol_vec = mol_vec_old.sum(dim=0) / a_size
                elif self.aggregation == 'param_self_attention':
                    param_self_attn = Parametric_attention(mol_vec.shape[-1], mol_vec.shape[-1])
                    output, attn_weights = param_self_attn.forward(mol_vec)
                    mol_vec = output.sum(dim=0) / a_size #squeeze() b4 sum
                elif self.aggregation == 'multihead_attention':
                    mol_vec = mol_vec.unsqueeze(0) #test with this new thing just a separate experiment
                    multihead = MultiHeadAttention(d_model=mol_vec.shape[-1], heads=5)
                    output =multihead.forward((mol_vec))
                    mol_vec = output.squeeze().sum(dim=0) / a_size
                elif self.aggregation == 'transformer':
                    mol_vec = mol_vec.unsqueeze(0)
                    model_encoder = TransformerEncoder(d_model=mol_vec.shape[-1], num_heads=5, num_layers=4)
                    hidden, output = model_encoder(mol_vec)
                    mol_vec = output.squeeze().sum(dim=0) / a_size
